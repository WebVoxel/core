...
export class Noise {
    constructor(seed: number = Math.random()) {
        if(seed > 0 && seed < 1) seed *= 65536; // Scale the seed out
    ...
    perlin2d(x: number, y: number): number {
        ...
        let xFloor = Math.floor(x), yFloor = Math.floor(y);
        // Get relative xy coordinates of point within that cell
        x = x - xFloor; y = y - yFloor;
        // Wrap the integer cells at 255 (smaller integer period can be introduced here)
        xFloor = xFloor & 255; yFloor = yFloor & 255;

        // Calculate noise contributions from each of the four corners
        let n00 = gradP[xFloor+perm[yFloor]].dot2(x, y);
        let n01 = gradP[xFloor+perm[yFloor+1]].dot2(x, y-1);
        let n10 = gradP[xFloor+1+perm[yFloor]].dot2(x-1, y);
        let n11 = gradP[xFloor+1+perm[yFloor+1]].dot2(x-1, y-1);
